...
fn single_number_argument(args: Vec<XPathValue>, function: &str) -> Result<f64> {
    require_arguments(&args, function, 1)?;
    Ok(args.into_iter().next().unwrap().to_number())
}
...
pub fn fn_floor(args: Vec<XPathValue>, _ctx: &EvaluationContext<'_>) -> Result<XPathValue> {
    let value = single_number_argument(args, "floor")?;
...
pub fn fn_ceiling(args: Vec<XPathValue>, _ctx: &EvaluationContext<'_>) -> Result<XPathValue> {
    let value = single_number_argument(args, "ceiling")?;
...
pub fn fn_round(args: Vec<XPathValue>, _ctx: &EvaluationContext<'_>) -> Result<XPathValue> {
    let value = single_number_argument(args, "round")?;
...
mod tests {
    ...
    fn test_fn_number_with_arg() {
        let result = evaluate("number", vec![XPathValue::String("42.5".to_string())]).unwrap();
    ...
    fn test_fn_sum_empty() {
        let result = evaluate("sum", vec![XPathValue::NodeSet(vec![])]).unwrap();
    ...
    fn test_fn_sum_wrong_args() {
        // No arguments
        let result = evaluate("sum", vec![]);
        assert!(result.is_err());

        // Wrong type
        let result = evaluate("sum", vec![XPathValue::Number(42.0)]);
        assert!(result.is_err());
    }

    #[test]
    fn test_fn_floor() {
        let result = evaluate("floor", vec![XPathValue::Number(2.9)]).unwrap();
        assert_eq!(result.to_number(), 2.0);

        let result = evaluate("floor", vec![XPathValue::Number(-2.1)]).unwrap();
        assert_eq!(result.to_number(), -3.0);
    }

    #[test]
    fn test_fn_floor_wrong_args() {
        let result = evaluate("floor", vec![]);
        assert!(result.is_err());
    }

    #[test]
    fn test_fn_ceiling() {
        let result = evaluate("ceiling", vec![XPathValue::Number(2.1)]).unwrap();
        assert_eq!(result.to_number(), 3.0);

        let result = evaluate("ceiling", vec![XPathValue::Number(-2.9)]).unwrap();
        assert_eq!(result.to_number(), -2.0);
    }

    #[test]
    fn test_fn_ceiling_wrong_args() {
        let result = evaluate("ceiling", vec![]);
        assert!(result.is_err());
    }

    #[test]
    fn test_fn_round_basic() {
        let result = evaluate("round", vec![XPathValue::Number(1.5)]).unwrap();
        assert_eq!(result.to_number(), 2.0);

        let result = evaluate("round", vec![XPathValue::Number(2.5)]).unwrap();
        assert_eq!(result.to_number(), 3.0);
    }

    #[test]
    fn test_fn_round_negative() {
        let result = evaluate("round", vec![XPathValue::Number(-0.5)]).unwrap();
        assert_eq!(result.to_number(), 0.0);
    ...
    fn test_fn_round_special_values() {
        let result = evaluate("round", vec![XPathValue::Number(f64::NAN)]).unwrap();
        assert!(result.to_number().is_nan());

        let result = evaluate("round", vec![XPathValue::Number(f64::INFINITY)]).unwrap();
        assert!(result.to_number().is_infinite());

        let result = evaluate("round", vec![XPathValue::Number(0.0)]).unwrap();
        assert_eq!(result.to_number(), 0.0);
    }

    #[test]
    fn test_fn_round_wrong_args() {
        let result = evaluate("round", vec![]);
        assert!(result.is_err());
    }
}
